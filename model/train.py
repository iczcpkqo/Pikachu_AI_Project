...
def loadData(fileNum=0, filePathBase="d:/traing_data/"):
...
def trainModel(loadPrev=True):
    ...
    for i in range(numFiles):
        ...
        model.fit(x=X, y=Y, epochs=epochs, batch_size=batchSize, validation_split=0.01, callbacks=callbacks)
...
def predict(stickers):
    r = stickers.reshape(6,3,3)
    c = Cube(r)
    stickers = stickers.reshape((1, 54))
    print(c.stickers)
    ...
    while not c.isSolved():
        ...
        pred = predictMove(stickers, getTrainedModel())
        soln += " " + pred
        c(pred)
